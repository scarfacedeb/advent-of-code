...
def move(stacks, m, order = nil)
  stacks[m[:from]].shift(m[:cnt]).then { stacks[m[:to]].unshift(*(order == :stack ? _1.reverse : _1)) }
end
...
moves = raw_moves.flatten.map { |m| m.match(/move (\d+) from (\d+) to (\d+)/) { { cnt: _1[1].to_i, from: _1[2].to_i - 1, to: _1[3].to_i - 1 } } }
